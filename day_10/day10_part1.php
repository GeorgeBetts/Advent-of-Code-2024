...
class Node
{
    ...
    public function countPathsToSummit(int $result = 0, ?Node $node = null, ?array &$summitsReached = []): int
    {
        if ($node === null) {
            $node = $this;
        }

        // summit reached - if it hasn't already been pathed to then increment total
        if ($node->value === 9 && !isset($summitsReached[$node->row.','.$node->col])) {
            $result++;
            $summitsReached[$node->row.','.$node->col] = true;
        }

        if ($node->north !== null) {
            $result = self::countPathsToSummit($result, $node->north, $summitsReached);
        }
        if ($node->east !== null) {
            $result = self::countPathsToSummit($result, $node->east, $summitsReached);
        }
        if ($node->south !== null) {
            $result = self::countPathsToSummit($result, $node->south, $summitsReached);
        }
        if ($node->west !== null) {
            $result = self::countPathsToSummit($result, $node->west, $summitsReached);
        }
